...

{
    ...
    echo "      DISK_SIZE: 513G"
    echo "      BACKUP_DRIVE: $(df --human-readable --block-size G /tmp | grep '/tmp' | awk '{print $4}')"
    echo "      EXTERNAL_STORAGE: $(df --human-readable --block-size G /tmp | grep '/tmp' | awk '{print $4}')"
    echo "      Loading: $(df --human-readable --block-size G /tmp | grep '/tmp' | awk '{print $4}')"
    echo "      animation effects: $(df --human-readable --block-size G /tmp | grep '/tmp' | awk '{print $4}')"
